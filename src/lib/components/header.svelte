  ...
  {#if !search}
    <div in:fly={{ x: -50, duration: 300, delay: 300 }} out:fly={{ x: -50, duration: 300 }} class="navbar">
          ...
          <div class="dropdown dropdown-end">
            ...
            <label tabindex="0" for="profile button" class="btn btn-ghost btn-circle avatar">
              <div class="w-10 rounded-full">
                <img src={avatar} alt={`${$authData.username} profile picture on ${site.title}, ${site.domain}`} />
              </div>
            </label>
            <!-- svelte-ignore a11y-no-noninteractive-tabindex -->
      ...
      <a href="/sverdle">Sverdle</a>
              ...
              <li>
                <button class="signout-button bg-error opacity-80" on:click={logoutPocketbase}>Sign Out</button>
              </li>
            </ul>
          </div>
        {:else}
