...
const render = async (/** @type {number} */ page, /** @type {any} */ url) =>
  ...
  <urlset
    ...
    ${await genMangaPosts(page).then((mangas) =>
			mangas
				.map(
					(
						/** @type {{ url: any; image: any; description: string | number | boolean; title: string | number | boolean; }} */ manga
					) =>
						`
...
const pingGoogle = async (/** @type {number} */ page, /** @type {string} */ url) => {
 ...
 * @param {string} url
 */
async function indexer(page, url) {
